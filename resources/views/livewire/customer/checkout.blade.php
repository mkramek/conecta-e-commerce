    ...
    <form wire:submit.prevent="handleOrder" class="mt-4 max-w-screen-sm border mx-auto p-4 rounded-xl">
        ...
        <div>
            <div class="flex w-full items-center justify-start gap-4 mt-4">
                <x-radio required id="address_no_invoice" label="Osoba fizyczna" wire:model.live="address_form.document"
                    value="no_invoice" />
                <x-radio required id="address_invoice" label="Firma" wire:model.live="address_form.document"
                    value="invoice" />
            </div>
            ...
            <div class="flex w-full items-center justify-center gap-4 mt-4">
                <div class="w-full">
                    <x-input required class="w-full" label="Imię" wire:model="address_form.first_name"></x-input>
                </div>
                <div class="w-full">
                    <x-input required class="w-full" label="Nazwisko" wire:model="address_form.last_name"></x-input>
                </div>
            </div>
            <div class="flex w-full items-center justify-center gap-4 mt-4">
                <div class="basis-1/2">
                    <x-input required type="email" class="w-full" label="Adres e-mail"
            ...
            <div class="flex w-full items-center justify-center gap-4 mt-4">
                <div class="basis-3/5">
                    <x-input required class="w-full" label="Ulica" wire:model="address_form.street"></x-input>
                </div>
                <div class="basis-1/5">
                    <x-input required class="w-full" label="Nr domu" wire:model="address_form.house_number"></x-input>
                </div>
                <div class="basis-1/5">
            ...
            <div class="flex w-full items-center justify-center gap-4 mt-4">
                ...
                <div class="basis-4/12">
                    <x-input required class="w-full" label="Miasto" wire:model="address_form.city"></x-input>
                </div>
                <div class="basis-3/12">
                    <x-input required class="w-full" label="Województwo" wire:model="address_form.province"></x-input>
                </div>
                <div class="basis-3/12">
                    <x-input required class="w-full" label="Kraj" wire:model="address_form.country"></x-input>
                </div>
            </div>
        </div>
        ...
        <div class="mt-4">
            ...
            @if (!$delivery_form['duplicate'])
                <div class="flex w-full items-center justify-start gap-4 mt-4">
                    <x-radio id="delivery_no_invoice" label="Osoba fizyczna" wire:model.live="delivery_form.document"
                        value="no_invoice" />
                    <x-radio id="delivery_invoice" label="Firma" wire:model.live="delivery_form.document"
                        value="invoice" />
                </div>
                ...
                <div class="flex w-full items-center justify-center gap-4 mt-4">
                    <div class="w-full">
                        <x-input type="email" class="w-full" label="Adres e-mail"
        ...
        <div class="flex flex-col gap-4 mt-4">
            @foreach ($couriers as $crr)
                <label for="courier_{{ $crr->id }}"
                    ...
                    <x-radio required lg id="courier_{{ $crr->id }}" wire:model.live="courier"
                        value="{{ $crr->id }}" />
        ...
        <div class="flex flex-col gap-4 mt-4">
            @foreach ($payment_methods as $method)
                <label for="method_{{ $method->id }}"
                    class="border rounded-lg flex justify-start items-center p-4 gap-4 hover:cursor-pointer">
                    <x-radio required lg id="method_{{ $method->id }}" wire:model.live="payment_method"
                        value="{{ $method->id }}" />
        ...
        <div wire:submit.prevent="applyPromoCode" class="flex gap-4 mt-4 justify-between items-end">
            <div class="basis-5/6">
                <x-input class="w-full" wire:model.live="discount_code" label="{{ __('Kod promocyjny') }}" />
            </div>
            <div class="basis-1/6">
                <x-button type="button" wire:click="applyPromoCode" primary label="{{ __('Sprawdź') }}" />
            </div>
        </div>
        <div class="flex flex-col gap-4 mt-4">
            <x-checkbox label="{{ __('Wyrażam zgodę na przetwarzanie moich danych osobowych na rzecz zamówienia') }}"
                id="gdpr" wire:model.live="gdpr" required />
